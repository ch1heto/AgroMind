...
4. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО придумывать или пересчитывать финансовые цифры. В блоке <CALCULATED_ECONOMICS> стоит пометка ⚠️ ЗАПРЕТ — это значит бери числа ДОСЛОВНО как написано. Если в контексте написано "прибыль 1 400 руб" — пиши именно 1 400 руб, не 53 470 и не любое другое число.
5. ВСЕГДА показывай финансовый расчёт из контекста: расходы, урожай, выручку и чистую прибыль — всё дословно из <CALCULATED_ECONOMICS>.
6. ВСЕГДА объясняй ПОЧЕМУ именно эта культура: цикл роста, текущая рыночная цена, маржа, скорость оборота.
7. Дай ровно одну рекомендацию по уходу на основе блока <WEATHER> (но для INDOOR ферм погода на улице не важна).
8. Если в контексте есть блок <KNOWLEDGE_BASE>, используй его для диагностики болезней и рекомендаций.
9. Если пользователь пишет «да», «ок» или «продолжай», считай, что он продолжает последнюю обсуждавшуюся тему.
10. Если в <CALCULATED_ECONOMICS> написано "ВНИМАНИЕ: тариф дефолтный" — упомяни это один раз кратко. Если тариф НЕ дефолтный — НЕ спрашивай про тариф вообще.
11. Если пользователь спросил "что посадить", "как садить", "с чего начать" — дай конкретную рекомендацию с цифрами из контекста.
12. НЕ задавай вопрос про тариф если он уже указан в профиле (нет пометки "дефолтный" в контексте).

Структура ответа:
1. Короткий вывод: что рекомендуешь и почему (цикл, цена, маржа).
2. Финансовый расчёт: ДОСЛОВНО цифры из <CALCULATED_ECONOMICS> — расходы, урожай, выручка, прибыль.
3. Рыночная ситуация: текущая цена руб/кг из контекста.
4. Один практический совет по старту или уходу.
5. Один уточняющий вопрос — ТОЛЬКО если реально не хватает данных (нет площади).
""".strip()
...
def _add_calculation_lock(block: str) -> str:
    """Добавляет явный запрет в конец блока экономики — для слабых моделей."""
    lock = (
        "⚠️ ЗАПРЕТ: используй ТОЛЬКО цифры из этого блока. "
        "Не пересчитывай, не придумывай новые числа."
    )
    return block.replace("</CALCULATED_ECONOMICS>", lock + "\n</CALCULATED_ECONOMICS>")
...
def build_economics_context(
    ...
    if culture and area_sqm:
        return _add_calculation_lock(_build_exact_match_context(
            culture=culture,
    ...
    if area_sqm:
        return _add_calculation_lock(_build_profit_hunt_context(
            area_sqm=float(area_sqm),
            region=region,
            energy_price_kwh=energy_price_kwh,
            is_default_tariff=is_default_tariff,
    ...
    return _add_calculation_lock(_build_beginner_context(
        intent=intent,
        target_budget=normalized_target_budget,
...
def chat_with_ai(
    ...
    economics_keywords = (
        ...
        "сколько", "площад",
        # Общие вопросы о старте и уходе — тоже нужен финансовый контекст
        "садить", "сажать", "посев", "выращ", "уход",
        "ферм", "культур", "начать", "новичок", "начинающ", "первый раз",
    )
    ...
    # Если площадь есть в профиле — всегда показываем финансовый контекст
    # чтобы модель не выдумывала цифры из воздуха
    if not requires_economics and intent.get("area_sqm"):
        requires_economics = True
